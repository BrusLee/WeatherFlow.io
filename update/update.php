<?php
    ...
    elseif($type=="wu"){
        ...
        $wuURL = "https://www.wunderground.com/weatherstation/WXDailyHistory.asp?ID=".$wuID."&format=XML";
        ...
        $xmlWU = simplexml_load_string($fileRaw);
        // print_r($xmlWU);
        $updateLog['info'][] = "XML data received from WU";
        // get latest observation
        $lastWU = count($xmlWU->current_observation) -1;
        if($lastWU == -1){
            $updateLog['error'][] = "Data received from WU has no elements";
            generateUpdateLog();
            die("Data received from WU has no elements");
        }
        $rawUpdate['SW'] = "WU";
        $updateLog['info'][] = "Loaded data items from WU = ".$lastWU;
        $rawUpdate['U'] = strtotime($xmlWU->current_observation[$lastWU]->observation_time_rfc822);
        $rawUpdate['T'] = (string)$xmlWU->current_observation[$lastWU]->temp_c;
        $rawUpdate['H'] = (string)$xmlWU->current_observation[$lastWU]->relative_humidity;
        $rawUpdate['P'] = (string)$xmlWU->current_observation[$lastWU]->pressure_mb;
        $rawUpdate['W'] = (string)$xmlWU->current_observation[$lastWU]->wind_mph;
        $rawUpdate['W'] = convertor($rawUpdate['W'],"mph","kmh");
        $rawUpdate['G'] = (string)$xmlWU->current_observation[$lastWU]->wind_gust_mph;
        $rawUpdate['G'] = convertor($rawUpdate['G'],"mph","kmh");
        $rawUpdate['R'] = (string)$xmlWU->current_observation[$lastWU]->precip_today_in;
        $rawUpdate['R'] = convertor($rawUpdate['R'],"in","mm");
        $rawUpdate['RR'] = (string)$xmlWU->current_observation[$lastWU]->precip_1hr_in;
