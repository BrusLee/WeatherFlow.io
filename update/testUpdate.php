<?php
	...
	if($parameters['type'] == "wu"){
		...
		$dataRaw = file_get_contents('http://www.wunderground.com/weatherstation/WXDailyHistory.asp?ID='.$WUID.'&format=XML');
		$xml = simplexml_load_string($dataRaw);
		
		if($dataRaw==""){
			die("Unable to load conditions from weatherunderground.com. Contact me (Jachym).");
		}
		
		$last = count($xml->current_observation) - 1;
		$dateTimeFormatted = strtotime($xml->current_observation[$last]->observation_time_rfc822);
		
		echo "Parsing data...<br><br>";
		 
		// use standardized field numbers and units
		$Traw = (string)$xml->current_observation[$last]->temp_c; // in C
		$Hraw = (string)$xml->current_observation[$last]->relative_humidity;
		$Praw = (string)$xml->current_observation[$last]->pressure_mb;
		$Wraw = (string)$xml->current_observation[$last]->wind_mph;
		$Graw = (string)$xml->current_observation[$last]->wind_gust_mph;
		$Rraw = (string)$xml->current_observation[$last]->precip_today_in;
		$RRraw = (string)$xml->current_observation[$last]->precip_1hr_in;
		$Braw = (string)$xml->current_observation[$last]->wind_degrees;
		if($solarSensor){
			$Sraw = (string)$xml->current_observation[$last]->solar_radiation;
		}
		...
		echo "Raw value: ".$Wraw."<br>";
		echo "Database wind speed units: ".$dataWindUnits."<br>";
		echo "File wind speed units: mph<br>";
		...
		$W = convertor($Wraw,"mph",$dataWindUnits);
		$W = number_format($W,1,".","");
		echo "Final wind speed value: ".$W." ".$dataWindUnits."<br><br>";
		...
		echo "Raw value: ".$Graw."<br>";
		echo "Database wind speed units: ".$dataWindUnits."<br>";
		echo "File wind gust units: mph<br>";
		if($dataWindUnits=="mph"){
			echo "Same units, no need to do any conversion.<br>";
		}
		else{
		...
		$G = convertor($Graw,"mph",$dataWindUnits);
		$G = number_format($G,1,".","");
		echo "Final wind speed value: ".$G." ".$dataWindUnits."<br><br>";
		...
		$R = convertor($Rraw,"in",$dataRainUnits);
		$R = number_format($R,2,".","");
		echo "Final precipitation value: ".$R." ".$dataRainUnits."<br><br>";
		...
		echo "Raw value: ".$Rraw."<br>";
		echo "Database precipitation units: ".$dataRainUnits."/h<br>";
		echo "File precipitation units: in/h<br>";
		if($dataRainUnits=="in"){
			echo "Same units, no need to do any conversion, only convert to hour rate.<br>";
		}
		else{
		...
		$RR = convertor($RRraw,"in",$dataRainUnits);
		$RR = number_format($RR,2,".","");
		echo "Final rain rate value: ".$RR." ".$dataRainUnits."/h<br><br>";
